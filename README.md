...
| `@amond-ai/harness-protocol` | both | The bridge wire schema: every frame a host emits and the orchestrator sends, as zod schemas. One entry point per harness — `./claude-code` for Claude, `./codex` for Codex — over a root holding only the halves they share, so each sandbox bundle carries only its own adapter's schemas. |
