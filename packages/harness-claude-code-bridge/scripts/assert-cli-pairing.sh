...
# packages/harness-claude-code-bridge/package.json to the same patch. Regenerate the root lockfile
...
if [ "${cli_release}" != "${cli_line}" ] || [ "${sdk_release}" != "${sdk_line}" ] \
  ...
  echo "       pin packages/harness-claude-code-bridge/package.json to ${sdk_line}.${cli_patch}" >&2
  exit 1
fi
