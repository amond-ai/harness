 ...
 *
 * Kept pure — snapshot in, events out — so the projection can be exercised without a sandbox.
 * Reading the files is the session's job.
 *
 * The cursor encodes how far each stream has been served (`"<stdoutBytes>:<stderrBytes>"`). It
 * is opaque to callers: `readLogSample` in the run workflow only ever round-trips it back
 * through `since`, and an unparseable one is read as the start rather than rejected, because a
 * replay that silently returns nothing would read as a wedged turn (AC-026).
 *
 ...
 *
 * The cursor still encodes the files' full lengths, not how much was served — it is a position
 * in the journal, and `data.length` is only the part that came after the last one. No terminal
 * event, for the reason the module docblock gives.
 */
